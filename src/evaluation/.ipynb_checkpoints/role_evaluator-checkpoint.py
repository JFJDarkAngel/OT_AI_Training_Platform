...
from src.llm.client import TEXT_MODEL, get_openai_client
...
def validate_evaluation_references(
    ...
    validated_references: list[EvaluationReference] = []

    for reference in evaluation.references:
        source_chunk = allowed_chunks.get(reference.chunk_id)

        if source_chunk is None:
            continue

        metadata = source_chunk.get("metadata") or {}

        page_number = metadata.get("page_number")

        try:
            validated_page_number = int(page_number)
        ...
        if not relevance:
            relevance = (
                "This retrieved source supports the evaluation."
            )
...
def evaluate_role_response(
    ...
    if evaluation.stakeholder != cleaned_stakeholder:
        raise ValueError(
            "Returned stakeholder does not match the "
            f"requested stakeholder: {cleaned_stakeholder}"
        )
